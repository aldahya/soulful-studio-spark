import { useEffect, useRef, useState } from 'react';
  ...
  import { ArrowRight, Printer, MessageCircle, Loader2, TrendingUp, AlertTriangle } from 'lucide-react';
  ...
  export default function StudentDetail() {
    ...
    const attendanceRate = totalRecorded > 0 ? Math.round(((stats.present + stats.late) / totalRecorded) * 100) : null;
    const rateColor = attendanceRate === null ? 'text-muted-foreground' : attendanceRate >= 80 ? 'text-success' : attendanceRate >= 65 ? 'text-warning' : 'text-destructive';
    const rateBorderClass = attendanceRate === null ? '' : attendanceRate >= 80 ? 'border-success/40 bg-success/5' : attendanceRate >= 65 ? 'border-warning/40 bg-warning/5' : 'border-destructive/40 bg-destructive/5';
    const rateBarColor = attendanceRate === null ? '' : attendanceRate >= 80 ? 'hsl(var(--success))' : attendanceRate >= 65 ? 'hsl(var(--warning))' : 'hsl(var(--destructive))';
    const alertHigh = attendanceRate !== null && attendanceRate < 75;
    ...
    return (
      <div className="space-y-6">
        ...
        {alertHigh && (
          <div className="flex items-center gap-3 rounded-xl border border-destructive/40 bg-destructive/10 px-4 py-3 text-destructive">
            ...
            <p className="text-sm font-medium">
              تنبيه: نسبة حضور هذا الطالب منخفضة ({attendanceRate}%) — يُنصح بمراجعة الولي.
            </p>
          </div>
        )}
        ...
        {/* Stats: 4 cards including attendance rate */}
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <Card className="p-5 shadow-soft">
            <p className="text-xs text-muted-foreground">حاضر</p>
            <p className="mt-2 text-3xl font-extrabold text-success">{stats.present}</p>
            <p className="mt-1 text-xs text-muted-foreground">يوم</p>
          </Card>
          <Card className="p-5 shadow-soft">
            <p className="text-xs text-muted-foreground">متأخر</p>
            <p className="mt-2 text-3xl font-extrabold text-warning">{stats.late}</p>
            <p className="mt-1 text-xs text-muted-foreground">يوم</p>
          </Card>
          <Card className="p-5 shadow-soft">
            <p className="text-xs text-muted-foreground">غائب</p>
            <p className="mt-2 text-3xl font-extrabold text-destructive">{stats.absent}</p>
            <p className="mt-1 text-xs text-muted-foreground">يوم</p>
          </Card>
          <Card className={`p-5 shadow-soft border-2 ${rateBorderClass}`}>
            <div className="flex items-center justify-between">
              <p className="text-xs text-muted-foreground">نسبة الحضور</p>
              <TrendingUp className={`h-4 w-4 ${rateColor}`} />
            </div>
            <p className={`mt-2 text-3xl font-extrabold ${rateColor}`}>
            ...
            {attendanceRate !== null && (
              <div className="mt-2 h-2 overflow-hidden rounded-full bg-muted">
                <div
                  ...
                  style={{ width: `${attendanceRate}%`, background: rateBarColor }}
                />
              </div>
            )}
            <p className="mt-1 text-xs text-muted-foreground">من {totalRecorded} يوم مُسجَّل</p>
          </Card>
        </div>
