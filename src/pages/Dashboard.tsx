import { useEffect, useMemo, useState } from 'react';
  ...
  export default function Dashboard() {
    ...
    async function loadAll() {
      ...
      // 7-day window
      const week7Start = new Date();
      week7Start.setDate(week7Start.getDate() - 6);
      const week7ISO = week7Start.toISOString().slice(0, 10);

      // 30-day window
      const month30Start = new Date();
      month30Start.setDate(month30Start.getDate() - 29);
      const month30ISO = month30Start.toISOString().slice(0, 10);
      ...
      // Build 7-day trend
      const week7Map = new Map<string, DayPoint>();
      for (let i = 0; i < 7; i++) {
        const d = new Date(week7Start);
        d.setDate(week7Start.getDate() + i);
        const key = d.toISOString().slice(0, 10);
        const label = new Intl.DateTimeFormat('ar-SA-u-ca-gregory', { weekday: 'short' }).format(d);
        week7Map.set(key, { day: label, حاضر: 0, متأخر: 0, غائب: 0 });
      }
      (weekAtt.data ?? []).forEach((r: any) => {
        const point = week7Map.get(r.date);
        if (!point) return;
        if (r.status === 'present') point['حاضر']++;
        else if (r.status === 'late') point['متأخر']++;
        else if (r.status === 'absent') point['غائب']++;
      });
      setWeekTrend(Array.from(week7Map.values()));

      // Build 30-day trend — group by week label for readability
      const month30Map = new Map<string, DayPoint>();
      for (let i = 0; i < 30; i++) {
        const d = new Date(month30Start);
        d.setDate(month30Start.getDate() + i);
        const key = d.toISOString().slice(0, 10);
        const day = d.getDate();
        const month = d.getMonth() + 1;
        const label = `${day}/${month}`;
        month30Map.set(key, { day: label, حاضر: 0, متأخر: 0, غائب: 0 });
      }
      (monthAtt.data ?? []).forEach((r: any) => {
        const point = month30Map.get(r.date);
        if (!point) return;
        if (r.status === 'present') point['حاضر']++;
        else if (r.status === 'late') point['متأخر']++;
        else if (r.status === 'absent') point['غائب']++;
      });
      setMonthTrend(Array.from(month30Map.values()));
    ...
    const monthSummary = useMemo(() => {
      const total = monthTrend.reduce((acc, d) => acc + d['حاضر'] + d['متأخر'] + d['غائب'], 0);
      const present = monthTrend.reduce((acc, d) => acc + d['حاضر'] + d['متأخر'], 0);
    ...
    const trendData = trendPeriod === 'week' ? weekTrend : monthTrend;
    ...
    return (
      <div className="space-y-8">
        ...
        {/* Trend Charts — week / month toggle */}
        <section className="grid gap-4 lg:grid-cols-3">
          <Card className="p-6 shadow-soft lg:col-span-2">
            <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
              <div>
                <h3 className="font-bold">اتجاه الحضور</h3>
                <p className="text-xs text-muted-foreground">
                  {trendPeriod === 'week' ? 'آخر 7 أيام' : `آخر 30 يوماً — متوسط الحضور ${monthSummary}%`}
                </p>
              </div>
              <div className="flex gap-1 rounded-lg border border-border/60 p-1">
                <button
                  onClick={() => setTrendPeriod('week')}
                  className={`rounded-md px-3 py-1.5 text-xs font-semibold transition-all ${trendPeriod === 'week' ? 'bg-primary text-primary-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}
                >
                  أسبوعي
                </button>
                <button
                  onClick={() => setTrendPeriod('month')}
                  className={`rounded-md px-3 py-1.5 text-xs font-semibold transition-all ${trendPeriod === 'month' ? 'bg-primary text-primary-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}
                >
                  شهري
                </button>
              </div>
              ...
              <ResponsiveContainer width="100%" height="100%">
                {trendPeriod === 'week' ? (
                  <AreaChart data={trendData} margin={{ top: 8, right: 12, left: 0, bottom: 0 }}>
                    ...
                    <Tooltip contentStyle={{ background: 'hsl(var(--card))', border: '1px solid hsl(var(--border))', borderRadius: 12, fontFamily: 'Cairo' }} />
                  ...
                  <BarChart data={trendData} margin={{ top: 8, right: 12, left: 0, bottom: 0 }}>
                    ...
                    <Bar dataKey="حاضر" stackId="a" fill="hsl(var(--success))" radius={[0, 0, 0, 0]} />
                    <Bar dataKey="متأخر" stackId="a" fill="hsl(var(--warning))" radius={[0, 0, 0, 0]} />
                    <Bar dataKey="غائب" stackId="a" fill="hsl(var(--destructive))" radius={[4, 4, 0, 0]} />
                  </BarChart>
                )}
              </ResponsiveContainer>
            </div>
